import random, math,time,os,base64
...
class vieCode:
    ...
    __fontPatn = os.path.join(os.getcwd(),'static','font','1.ttf') #字体
    ...
    def GetCodeImage(self,size = 80,length = 4):
        ...
        return self.__img,(''.join(self.__code)).upper()
    def GetCodeImageBase64(self):
        img,string = self.GetCodeImage()
        imgName = str(time.time()+random.random())+'.png'
        img.save(imgName)
        with open(imgName,'rb') as f:
            imgBase64 = base64.b64encode(f.read())
        os.remove(imgName)
        return [imgBase64.decode(),string]
